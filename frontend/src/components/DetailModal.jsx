...
export default function DetailModal({ item, onClose }) {
  ...
  const isCloudItem =
    item.category === "cloud" || (item.offers && item.offers.length > 0);
  const currentTab =
    activeTab === "auto" ? (isCloudItem ? "cloud" : "specs") : activeTab;
  ...
  const price =
    item.price ||
    (item.onDemandUsd
      ? `$${item.onDemandUsd.toFixed(2)}/hr`
      : "₹4.98L - ₹7.50L");

  // Cloud offers data processing
  const secureOffers = item.offers
    ? item.offers.filter((o) => o.kind === "secure" || o.kind === "on-demand")
    : [];
  const communityOffers = item.offers
    ? item.offers.filter((o) => o.kind === "community" || o.kind === "spot")
    : [];
  const allOffers =
    item.offers && item.offers.length > 0
      ...
      : [
          ...
          },
          {
            ...
            usdHr: 2.49,
            fetchedAt: "2026-07-26",
            sourceUrl: "https://lambdalabs.com",
            sourceDomain: "lambdalabs.com",
          },
          {
            variant: name,
            provider: "CoreWeave",
            ...
            usdHr: 2.6,
            fetchedAt: "2026-07-26",
            sourceUrl: "https://coreweave.com",
            sourceDomain: "coreweave.com",
          },
        ];
  ...
  const cheapestOnDemand =
    onDemandRates.length > 0 ? onDemandRates[0] : item.onDemandUsd || 2.39;
  const cheapestSpot =
    communityRates.length > 0 ? communityRates[0] : item.spotUsd || 0;
  const maxOnDemand =
    onDemandRates.length > 0
      ? onDemandRates[onDemandRates.length - 1]
      : cheapestOnDemand;
  ...
  if (onDemandRates.length > 0) {
    ...
    medianOnDemand =
      onDemandRates.length % 2 !== 0
        ? onDemandRates[mid]
        : (onDemandRates[mid - 1] + onDemandRates[mid]) / 2;
  }

  const uniqueProvidersCount = new Set(
    allOffers.map((o) => (o.provider ? o.provider.toLowerCase() : "")),
  ).size;
  const cheapestOffer =
    secureOffers.length > 0
      ? secureOffers[0]
      : allOffers.length > 0
        ? allOffers[0]
        : null;
  const cheapestProvider = cheapestOffer
    ? cheapestOffer.provider
    : item.cheapestProvider || item.where || "Runpod";
  const sourceDomain = cheapestOffer
    ? cheapestOffer.sourceDomain || "runpod.io"
    : "runpod.io";
  const verifiedDate = item.verifiedDate || "2026-07-26";
  const vendor =
    name.toUpperCase().includes("MI") ||
    name.toUpperCase().includes("AMD") ||
    name.toUpperCase().includes("RADEON")
      ? "AMD"
      : "NVIDIA";
  ...
  const cloudRate = item.spotUsd
    ? `$${item.spotUsd.toFixed(2)}/hr`
    : `$${cheapestOnDemand.toFixed(2)}/hr (₹369/hr)`;
          ...
          >
            <button
              ...
              style={{
                ...
                boxShadow:
                  currentTab === "cloud" ? "var(--shadow-subtle-1)" : "none",
                cursor: "pointer",
              }}
            >
              ...
              style={{
                ...
                color:
                  currentTab === "specs"
                    ...
                    : "var(--color-charcoal-stone)",
                boxShadow:
                  currentTab === "specs" ? "var(--shadow-subtle-1)" : "none",
                cursor: "pointer",
              }}
            >
        ...
        {currentTab === "cloud" && (
          <div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                The cheapest on-demand {name} rental today is{" "}
                <strong
                  style={{ fontWeight: 700, color: "var(--color-ink-black)" }}
                >
                  ${cheapestOnDemand.toFixed(2)}/hr
                </strong>{" "}
                on {cheapestProvider} ({cheapestOffer?.kind || "secure"}):
                verified {verifiedDate}. {vram} VRAM, {vendor}.
              </p>
            </div>
            ...
            >
              ...
              >
                ...
                {cheapestSpot > 0 &&
                  `(spot/community from $${cheapestSpot.toFixed(2)})`}{" "}
                · {cheapestProvider} ({cheapestOffer?.kind || "secure"}) ·
                source:{" "}
                <a
        ...
        {currentTab === "specs" && (
          <div>
            ...
            >
              ...
              >
                <span style={{ color: "var(--color-charcoal-stone)" }}>
                  Capacity
                </span>
                <span
                  style={{
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div className="grid-2col" style={{ gap: 16 }}>
                ...
                >
                  ...
                  >
                    Available at ~{cloudRate}. Breakeven point reached after{" "}
                    <strong
                      style={{
                        fontWeight: 700,
                        color: "var(--color-ink-black)",
                    ...
                    of execution compared to direct CapEx.
                  </p>
                </div>
                ...
                <div className="card-paper-white" style={{ padding: 20 }}>
                  ...
                  >
                    Can be integrated into a Custom AI Workstation starting from{" "}
                    <strong
                      style={{
                        fontWeight: 700,
                        color: "var(--color-ink-black)",
          ...
          <div className="grid-2col" style={{ gap: 16 }}>
            ...
            >
              <div>
                ...
                >
                  "Enterprise datacenter accelerator built specifically for
              ...
              >
                <div style={{ marginBottom: 4 }}>
                  ...
                  <span style={{ color: "var(--color-charcoal-stone)" }}>
                    Massive HBM bandwidth, high scalability.
                  </span>
                </div>
                <div>
                  <strong
                    style={{ color: "var(--color-coral-red)", fontWeight: 700 }}
                  >
                    CONS //{" "}
                  </strong>
                  <span style={{ color: "var(--color-charcoal-stone)" }}>
                    High rental cost, complex infrastructure required.
                  </span>
                </div>
              </div>
            </div>
