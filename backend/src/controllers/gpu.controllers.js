...
const groupOffersByGpu = (offers) => {
  ...
  offers.forEach((offer) => {
    ...
    groups[gpuKey].offers.push({
      ...
      provider:
        offer.provider.charAt(0).toUpperCase() + offer.provider.slice(1),
    ...
    if (offer.vram_gb && offer.vram_gb < groups[gpuKey].vramGbMin)
      groups[gpuKey].vramGbMin = offer.vram_gb;
    if (offer.vram_gb && offer.vram_gb > groups[gpuKey].vramGbMax)
      groups[gpuKey].vramGbMax = offer.vram_gb;
  });
  ...
  return Object.values(groups).map((group) => {
    ...
    const spotUsd =
      communityOffers.length > 0 ? communityOffers[0].usdHr : null;

    const cheapestOffer = group.offers[0];
    const cheapestProvider = cheapestOffer.provider;

    const vramLabel =
      group.vramGbMin === group.vramGbMax
        ? `${group.vramGbMin} GB`
...
const fetchRawLiveCloudData = async () => {
  ...
  try {
    ...
    console.error(
      "Live raw cloud fetch failed, falling back to static seed:",
      err.message,
    );
    return [];
  }
...
const getDbOffersFallback = () => {
  return cloudProviders.map((c) => ({
    provider: c.provider,
    gpu: c.gpu,
    vram_gb: c.gpu.toLowerCase().includes("5090")
      ? 32
      : c.gpu.toLowerCase().includes("4090")
        ? 24
        : c.gpu.toLowerCase().includes("h100")
          ? 80
          : c.gpu.toLowerCase().includes("a100")
            ? 40
            : 24,
    usd_hr: c.rateUsdHr,
    kind: c.profile.toLowerCase().includes("community")
      ? "community"
      : "secure",
    source_url: "#",
...
const getExternalSpecsFallback = (name) => {
  ...
  let rating = "8.6/10";
  let verdict =
    "A versatile scale node for mid-tier AI projects, providing balanced memory capacity.";
  ...
  if (clean.includes("5090")) {
    ...
    rating = "9.6/10";
    verdict =
      "The undisputed consumer flagship with 1.79 TB/s GDDR7 bandwidth across a 512-bit bus.";
    ...
    rating = "8.2/10";
    verdict =
      "Excellent portability for offline software testing, but limited by thermal boundaries.";
...
export const scrapeTechPowerUp = async (name) => {
  ...
  try {
    ...
    console.warn(
      `Real-time TechPowerUp fetch for "${name}" failed, using generator fallback:`,
      err.message,
    );
...
export const searchAll = asyncHandler(async (req, res) => {
  ...
  const matchedSystems = integratedSystems.filter(
    (s) => regex.test(s.type) || regex.test(s.gpu) || regex.test(s.specs),
  );
  ...
  const matchedCloud = groupedGpus.filter(
    (g) => regex.test(g.gpu) || regex.test(g.tier),
  );

  return res.status(200).json(
    new ApiResponse(200, "Search results", {
      gpus: matchedLocal.map((g) => ({
        ...g,
        category: "local",
        _id: `local-${g.name.toLowerCase().replace(/\s+/g, "-")}`,
      })),
      cloud: matchedCloud,
      systems: matchedSystems.map((s) => ({
        ...s,
        category: "system",
        _id: `system-${s.type.toLowerCase().replace(/\s+/g, "-")}`,
      })),
...
const parseSearchRows = (html) => {
  ...
  rows.forEach((row) => {
    ...
    const nameMatch = cells[0].match(
      /<div class="item-name"><a href="\/gpu-specs\/[^"]+">([^<]+)<\/a>/i,
    );
    if (!nameMatch) return;
    const name = nameMatch[1].trim();

    const chipMatch = cells[0].match(
      /<div class="item-chip"><a href="\/gpu-specs\/[^"]+">([^<]+)<\/a>/i,
    );
    ...
    const memTypeMatch = memory.match(/GDDR\d[X]?|HBM\d[e]?/i) || [
      null,
      "GDDR6",
    ];
    ...
    if (
      ...
      name.toLowerCase().includes("workstation") ||
      name.toLowerCase().includes("quadro") ||
      name.toLowerCase().includes("rtx a")
    ) {
...
export const getLocalGpus = asyncHandler(async (req, res) => {
  ...
  if (q && q.trim().length > 0) {
    ...
    const matchedSeed = localGpus.filter(
      (g) => reg.test(g.name) || reg.test(g.arch) || reg.test(g.gpuClass),
    );
    ...
    scraped.forEach((item) => {
      if (
        !combined.some((c) => c.name.toUpperCase() === item.name.toUpperCase())
      ) {
        combined.push(item);
      }
    });
...
export const getCloudProviders = asyncHandler(async (req, res) => {
  ...
  const stats = {
    ...
    cheapestH100Provider: cheapestH100Offer
      ? cheapestH100Offer.provider.charAt(0).toUpperCase() +
        cheapestH100Offer.provider.slice(1)
      : "Voltage Park",
  ...
  const providersList = [
    ...new Set(
      offers.map(
        (o) => o.provider.charAt(0).toUpperCase() + o.provider.slice(1),
      ),
    ),
  ].sort();
  ...
  return res.status(200).json(
    new ApiResponse(200, `Cloud Providers (${isLive ? "Live" : "Fallback"})`, {
      ...
      pagination: {
        page: 1,
        limit: groupedGpus.length,
        totalPages: 1,
        totalItems: groupedGpus.length,
      },
    }),
  );
});
...
export const getBandwidthData = asyncHandler(async (req, res) => {
  const items = [...localGpus].sort(
    (a, b) => a.bandwidthTbps - b.bandwidthTbps,
  );
  ...
  const data = items.map((g) => ({
    ...
    gpuClass: g.gpuClass,
    vramGb: g.vramGb,
    color: g.arch.includes("Blackwell")
      ? "#8b5cf6"
      : g.gpuClass.includes("Enterprise") ||
          g.gpuClass.includes("Datacenter") ||
          g.gpuClass.includes("LLM")
        ? "#0f172a"
        : "#94a3b8",
  }));
...
export const getExternalSpecs = asyncHandler(async (req, res) => {
  ...
  if (!name) {
    return res
      .status(400)
      .json(new ApiResponse(400, "GPU Name parameter is required"));
  }

  const result = await scrapeTechPowerUp(name);
  return res
    .status(200)
    .json(new ApiResponse(200, "External specs data", result));
});
